...
// QUICK TEST: Only 1 location and 1 calendar event
...
async function main(){
    try {
        console.log('=====================================');
        console.log('  QUICK 2-MINUTE TEST FLOW');
        console.log('=====================================\n');
        ...
        // Use only the first event for quick test
        const testEvents = calendarEvents.slice(0, 1);
        console.log(`✅ Using ${testEvents.length} event for quick test: ${testEvents[0].EventName}\n`);
...
async function loopCalendarEvents(currentLocation, currentAttorney, calendarEvents){
    for(const event of calendarEvents){
        try {
            console.log(`\n📅 Processing event: ${event.EventName} (ID: ${event.EventId})`);
            ...
            await sleep(10000); // Reduced wait time for quick test

            console.log('   📸 Taking screenshot...');
            const screenshotName = `${event.EventName.replace(/\s+/g, '-').toLowerCase()}`;
            await playWrightScreenshot(screenshotName, currentLocation);

            console.log(`   ✅ Successfully processed event: ${event.EventName}`);
        } catch (error) {
