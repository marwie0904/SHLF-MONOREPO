...
// NO NORMALIZATION - use exact values
const getName = (name) => name?.trim() || '';
const getPhone = (phone) => phone?.trim() || '';
...
contacts.forEach((contact, idx) => {
  const name = getName(contact['Contact Name']);
  if (name) {
...
for (const [name, indexes] of nameIndex.entries()) {
  ...
  for (const idx of indexes) {
    ...
    const primaryPhone = getPhone(contact['Primary Phone Number']);
    const secondaryPhone = getPhone(contact['Secondary Phone Number']);
    const primaryEmail = getEmail(contact['Primary Email Address']);
    const secondaryEmail = getEmail(contact['Secondary Email Address']);
    ...
    for (const group of groups) {
      ...
      for (const groupContact of group) {
        const gPrimaryPhone = getPhone(groupContact['Primary Phone Number']);
        const gSecondaryPhone = getPhone(groupContact['Secondary Phone Number']);
        const gPrimaryEmail = getEmail(groupContact['Primary Email Address']);
        const gSecondaryEmail = getEmail(groupContact['Secondary Email Address']);

        let hasPhoneMatch = false;
        let hasEmailMatch = false;

        // Check phone matches - EXACT match, must be non-empty
        if (primaryPhone && (primaryPhone === gPrimaryPhone || primaryPhone === gSecondaryPhone)) {
          hasPhoneMatch = true;
        }
        if (secondaryPhone && (secondaryPhone === gPrimaryPhone || secondaryPhone === gSecondaryPhone)) {
          hasPhoneMatch = true;
        }

        // Check email matches - EXACT match, must be non-empty
        if (primaryEmail && (primaryEmail === gPrimaryEmail || primaryEmail === gSecondaryEmail)) {
          hasEmailMatch = true;
        }
        if (secondaryEmail && (secondaryEmail === gPrimaryEmail || secondaryEmail === gSecondaryEmail)) {
          hasEmailMatch = true;
        }
...
if (allDuplicates.length > 0) {
  ...
  fs.writeFileSync('./csv/duplicate-contacts-exact.csv', outputCsv);
  console.log('Duplicates saved to ./csv/duplicate-contacts-exact.csv');
